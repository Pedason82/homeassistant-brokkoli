...
class PlantLocation(TextEntity, RestoreEntity):
    ...
    def __init__(self, hass: HomeAssistant, config: ConfigEntry, plant_device) -> None:
        ...
        if plant_device.device_type == DEVICE_TYPE_CYCLE:
            self._location = {"area": None, "areas": []}  # Liste für alle Member-Areas
        else:
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if last_state and last_state.state:
            try:
                ...
                if (
                    ...
                    and "areas" not in self._location
                ):
        ...
        self.async_on_remove(
            self.hass.bus.async_listen(
                "device_registry_updated", self._handle_area_change
            )
        )
            ...
            if area:
                ...
                if self._plant.device_type == DEVICE_TYPE_CYCLE:
                    if area.name not in self._location.get("areas", []):
                        self._location["areas"] = list(
                            set(self._location.get("areas", []) + [area.name])
                        )
                self._attr_native_value = json.dumps(self._location)
                self.async_write_ha_state()
    ...
    async def _propagate_area_to_members(self, area_id):
        ...
        for plant_id in self._plant._member_plants:
            ...
            if plant_device:
                ...
                device_registry.async_update_device(plant_device.id, area_id=area_id)
                _LOGGER.debug(
                    f"Updated area for member plant {plant_id} to match cycle"
                )
    ...
    def _handle_area_change(self, event):
        ...
        if event.data.get("action") == "update" and "area_id" in event.data.get(
            "changes", {}
        ):
            device_registry = dr.async_get(self._hass)
            area_registry = ar.async_get(self._hass)
                            ...
                            for dev in device_registry.devices.values():
                                if dev.id == device.via_device_id:
                                    ...
                                    for entry_id in self._hass.data[DOMAIN]:
                                        if (
                                            ATTR_PLANT
                                            in self._hass.data[DOMAIN][entry_id]
                                        ):
                                            cycle = self._hass.data[DOMAIN][entry_id][
                                                ATTR_PLANT
                                            ]
                                            if (
                                                cycle.device_type == DEVICE_TYPE_CYCLE
                                                and cycle.unique_id
                                                == next(iter(dev.identifiers))[1]
                                            ):
                                                from_cycle = True
